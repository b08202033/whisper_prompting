    ...
    def generate(
        ...
        **kwargs,
        ...
        if example_ids is not None:
            ...
            decoder_start_token_id, *text_example_ids = example_ids
            # Slicing the text prompt ids in a manner consistent with the OpenAI implementation
            # to accomodate context space for the prefix (see https://github.com/openai/whisper/blob/c09a7ae299c4c34c5839a76380ae407e7d785914/whisper/decoding.py#L599)
            text_example_ids = text_example_ids[-self.config.max_target_positions // 2 - 1 :]
            # Set the decoder_start_token_id to <|startofprev|>
            # kwargs.update({"decoder_start_token_id": decoder_start_token_id})
            ...
            # Reformat the forced_decoder_ids to incorporate the prompt
            non_example_forced_decoder_ids = (
                kwargs.pop("forced_decoder_ids", None) or generation_config.forced_decoder_ids
            )
            forced_decoder_ids = [
                *[token for _rank, token in non_example_forced_decoder_ids],
                *text_example_ids,
            ]
            forced_decoder_ids = [(rank + 1, token) for rank, token in enumerate(forced_decoder_ids)]
            generation_config.forced_decoder_ids = forced_decoder_ids
